...
var handleslotattack = function (msg,character,player) {
  ...
  const isUsingSpellPoints = getSpellPointFlag(character);
  if(spelllevel === "cantrip" || spelllevel === "npc" || innate || isUsingSpellPoints) {
      return;
  }
...
var handleslotspell = function (msg,character,player) {
  ...
  var innate = msg.content.indexOf("{{innate=}}") > -1 ? false : true;
  const isUsingSpellPoints = getSpellPointFlag(character);
  if(spellslot === "0" || spellslot === "cantrip" || spellslot === "npc" || ritual || innate || isUsingSpellPoints) {
      return;
  }
  resolveslot(msg,character,player,spellslot);
};
...
const getSpellPointFlag = (character) => {
  ...
  const spellPointFlag = getAttrByName(character.id, 'use_spell_points');
